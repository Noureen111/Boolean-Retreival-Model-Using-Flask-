...
def positional_index(stop_words):
    ...
    for i in range(1, 51):
        ...
        # stemming out
        for i in docs:
            stemmed_words.append(pstem.stem(i))
...
def index():
    ...
    if form.validate_on_submit():
        ...
        my_query = request.form.get("input")
...
def result():
    ...
    if '/' in my_query:
        ...
        result = process_proximity_query(my_query, positional_dictionary)

    else:
        my_query = decontract_words(my_query)
        my_query = my_query.lower()
        result = process_query(my_query, inverted_dictionary)
    
    result = list(result)
    result.sort()

    return render_template("result.html", result=list(result))
